# ============================================================
# ZOMBIE APOCALYPSE: THE MANSION — V5.0
# Text Adventure RPG | Davon Gass
# ============================================================
# WHAT'S NEW IN V5.0 vs V4.0 (Combat System — Pokémon-like):
#   - Full turn-based combat system
#   - 5 combat types: Brute, Alpha, Psyche, Bio, Stealth
#   - Type matchup chart (5×5 — type advantage/disadvantage)
#   - Player moves gated by Alpha Awareness stat
#   - Status effects: Blinded, Infected, Stressed, Steadied, Suppressed
#   - 3 enemy configs: Standard Zombie, Patient Zombie, Dr. Barbados
#   - Bathroom zombie: moral dilemma → actual combat encounter
#   - Living Room: scripted Standard Zombie encounter on first entry
#   - Barbados: full boss fight with type-advantage mechanics
#   - Use Item in combat (first aid kit, food)
#   - Combat stats added to Player (HP, attack, defense, type)
#   - Endings now determined after winning Barbados fight
#   - All V4 features fully preserved
# ============================================================
...
PLAYER_MOVES = {
    ...
    'flashlight_beam': Move(
        'Flashlight Beam', 'Psyche', 25, 0.85,
        effect='blinded', effect_chance=0.40,
        description='Chance to blind the target.'
    ),
    'static_pulse': Move(
...
def _on_pickup(item_name, player):
    ...
    if item_name == "flashlight":
        ...
        slow_print("[Flashlight Beam move now available in combat.]")
...
def encounter_barbados(player, rooms):
    ...
    # BEST ENDING: full truth + escape plan
    if knows_full and has_camp:
        slow_print("You know what you are now. What he made you.")
        slow_print("And you know there are people waiting.")
        print()
        if showed_mercy:
            slow_print("You showed mercy once today. This was a different call.")
    ...
    # SURVIVAL ENDING: got out, but missed the full picture
    else:
        slow_print("You're not sure exactly what just happened.")
        slow_print("You're not sure exactly what you are.")
        print()
        slow_print("But you're still standing.")
        slow_print("That has to count for something.")
        print()
        slow_print("You walk out into the grey morning.")
        print()
    ...
    while True:
        ...
        if verb == 'quit':
            ...
            if new_room != current:
                ...
                player.visited_rooms.add(current)
                action_enter_room(current, rooms, player)
                # V5: Scripted combat encounters on room entry
